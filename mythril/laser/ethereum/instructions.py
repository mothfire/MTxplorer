...
spotdata = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ123456'
...
class Instruction:
    ...
    def evaluate(self, global_state: GlobalState, post=False) -> List[GlobalState]:
        ...
        if global_state.firstexec:
            if global_state.mstate.sload:
                if op not in ['dup','swap','push','not','mul','div','exp','shl','shr','sub','and','signextend',
                              ]:
                    global_state.world_state.path.load_var_list.append((global_state.mstate.slot,0,32))
                    global_state.mstate.sload = False
                    global_state.mstate.stack2.clear()
    ...
    def jumpdest_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        mstate.jumpdest_list.append(mstate.pc)
        if global_state.firstexec:
            if mstate.jumpi:
                mstate.jumpi_flag = True
                mstate.jumpi = False
            else:
                mstate.jumpi_flag = False
        return [global_state]
    ...
    def push_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if global_state.firstexec:
            if global_state.mstate.sload:
                global_state.mstate.stack2.append(
                    symbol_factory.BitVecVal(int(push_value, 16), 256)
                )
        return [global_state]
    ...
    def dup_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if global_state.firstexec:
            if global_state.mstate.sload:
                global_state.mstate.stack2.append(global_state.mstate.stack2[-value])
        return [global_state]
    ...
    def swap_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if global_state.firstexec:
            if global_state.mstate.sload:
                stack2 = global_state.mstate.stack2
                stack2[-depth - 1], stack2[-1] = stack2[-1], stack2[-depth - 1]
        return [global_state]
    ...
    def and_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if global_state.firstexec:
            if global_state.mstate.sload:
                ...
                if op1.symbolic == False:
                    op1 = op1.value.to_bytes(length=32,byteorder='big').replace(b'\x00',b'')
                else:
                    op1 = b'None'
                if op2.symbolic == False:
                    op2 = op2.value.to_bytes(length=32,byteorder='big').replace(b'\x00',b'')
                else:
                    op2 = b'None'

                if op1 == spotdata.encode() or op2 == spotdata.encode() or op1 in spotdata.encode() or op2 in spotdata.encode():
                    r = stack2[-1]
                    if r.symbolic == False:
                        r = r.value.to_bytes(length=32,byteorder='big')

                        # get offset and size in storage slot - when OR,SWAP,SSTORE
                        z = r.count(b'\x00')
                        iz = r.index(b'\x00')
                        state.offset = 32 - (iz + z)
                        state.size = z

                        r = r.replace(b'\x00',b'').decode()
                        try:
                            i = spotdata.index(r)
                        except:
                            i = -1
                        ...
                        if o == 32 and l == 0:
                            return []
                        if o == None:
                            global_state.world_state.path.load_var_list.append((None,None,None))
                        else:
                            global_state.world_state.path.load_var_list.append((state.slot,o,l))
                    else:
                        pass
    ...
    def not_(self, global_state: GlobalState):
        ...
        if global_state.firstexec:
            if global_state.mstate.sload:
                mstate.stack2.append(TT256M1 - mstate.stack2.pop())
        return [global_state]
    ...
    def add_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if global_state.firstexec:
            if op1.symbolic == False and op2.symbolic == False:
                if op1.value in global_state.mstate.sha3_dist.keys():
                    global_state.mstate.sha3_dist[result.value] = global_state.mstate.sha3_dist[op1.value]
                if op2.value in global_state.mstate.sha3_dist.keys():
                    global_state.mstate.sha3_dist[result.value] = global_state.mstate.sha3_dist[op2.value]
        return [global_state]
        ...
        if global_state.firstexec:
            if global_state.mstate.sload:
                ...
                if isinstance(a1, Bool):
                    ...
                    a1 =  symbol_factory.BitVecVal(a1, 256)
                else:
                    a1 = cast(BitVec, a1)
                    a1 =  simplify(a1)
                if isinstance(a2, Bool):
                    ...
                    a2 = symbol_factory.BitVecVal(a2, 256)
                else:
                    a2 = cast(BitVec, a2)
                    a2 = simplify(a2)
                global_state.mstate.stack2.append(a1 - a2)
        return [global_state]
        ...
        if global_state.firstexec:
            if global_state.mstate.sload:
                ...
                if isinstance(a2, Bool):
                    ...
                    a2 = simplify(a2)
                if a1 == 0:
                    global_state.mstate.stack2.append(symbol_factory.BitVecVal(0, 256))
                else:
                    global_state.mstate.stack2.append(UDiv(a1, a2))
        ...
        if global_state.firstexec:
            if global_state.mstate.sload:
                ...
                if isinstance(a2, Bool):
                    ...
                    a2 = cast(BitVec, a2)
                    a2 = simplify(a2)
                global_state.mstate.stack2.append(a2 << a1)
        ...
        if global_state.firstexec:
            if global_state.mstate.sload:
                ...
                if isinstance(a2, Bool):
                    ...
                    a2 = cast(BitVec, a2)
                    a2 = simplify(a2)
                global_state.mstate.stack2.append(LShR(a2, a1))
    ...
    def exp_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if global_state.firstexec:
            if global_state.mstate.sload:
                ...
                state.stack2.append(exponentiation)
    ...
    def signextend_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if global_state.firstexec:
            if mstate.sload:
                s0, s1 = mstate.stack2.pop(), mstate.stack2.pop()
                ...
                if o == None:
                    ...
                    global_state.world_state.path.load_var_list.append((mstate.slot,o,l))
    ...
    def sha3_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if global_state.firstexec:
            if data.symbolic == False:
                if data.value < 65535:
                    state.sha3_dist[result.value] = data.value
                else:
                    if data.value in state.sha3_dist.keys():
                        state.sha3_dist[result.value] = state.sha3_dist[data.value]
            
            if len(data_list) > 32:
                if data.symbolic:
                    data_str = repr(data).replace('\n','').replace(' ','')
                    p = simplify(Concat(data_list[-32:]))
                    if p.symbolic == False and p.value < 65535:
                        global_state.mstate.sha3_map[data_str] = p.value
                else:
                    p = simplify(Concat(data_list[-32:]))
                    if p.symbolic == False:
                        if p.value < 65535:
                            state.sha3_dist[result.value] = p.value
                        else:
                            if p.value in state.sha3_dist.keys():
                                state.sha3_dist[result.value] = state.sha3_dist[p.value]
    ...
    def sload_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if global_state.firstexec:
            if index.symbolic == False:
                if index.value in global_state.mstate.sha3_dist.keys():
                    key = global_state.mstate.sha3_dist[index.value]
                    global_state.world_state.path.load_var_list.append((key,None,None))
                else:
                    state.stack2 = copy(state.stack)
                    state.stack2.pop()
                    n = int.from_bytes(spotdata.encode(),byteorder='big')
                    state.stack2.append(n)
                    state.sload = True
                    state.slot = index.value
                    state.offset = None
                    state.size = None
            else:
                index = repr(index).replace('\n','').replace(' ','')

                if 'keccak256' not in index:
                    sh3_key = state.sha3_dist.keys()
                    sflag = False
                    for k in sh3_key:
                        sk = str(k)
                        if sk in index:
                            key = global_state.mstate.sha3_dist[k]
                            global_state.world_state.path.load_var_list.append((key,None,None))
                            sflag = True
                    if not sflag:
                        global_state.world_state.path.load_var_list.append(('@',None,None))
                else:
                    map = global_state.mstate.sha3_map
                    for m in map.keys():
                        if m in index:
                            key = map[m]
                            global_state.world_state.path.load_var_list.append((key,None,None))
    ...
    def sstore_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if global_state.firstexec == None and index.symbolic == False:
            old_value = simplify(global_state.environment.active_account.storage[index])
            new_value = simplify(value)
            old_value = repr(old_value).replace('\n','').replace(' ','')
            new_value = repr(new_value).replace('\n','').replace(' ','')
            if old_value == new_value:
                return []
        global_state.environment.active_account.storage[index] = value
        if global_state.firstexec:
            instructions = global_state.environment.code.instruction_list
            if instructions[state.pc-2]['opcode'] == 'OR':
                if len(global_state.world_state.path.load_var_list) != 0:
                    global_state.world_state.path.load_var_list.pop()
                if index.symbolic == False:
                    if index.value in global_state.mstate.sha3_dist.keys():
                        key = global_state.mstate.sha3_dist[index.value]
                        if (key,None,None) not in global_state.world_state.path.store_var_list:
                            global_state.world_state.path.store_var_list.append((key,None,None))
                    else:
                        if (index.value,state.offset,state.size) not in global_state.world_state.path.store_var_list:
                            global_state.world_state.path.store_var_list.append((index.value,state.offset,state.size))
                            global_state.mstate.offset = None
                            global_state.mstate.size = None
                else:
                    index = repr(index).replace('\n','').replace(' ','')
                    ...
                    if 'keccak256' not in index:
                        ...
                        for k in sh3_key:
                            ...
                            if sk in index:
                                ...
                                sflag = True
                        if not sflag:
                            if ('@',None,None) not in global_state.world_state.path.store_var_list:
                                global_state.world_state.path.store_var_list.append(('@',None,None))
                    else:
                        map = global_state.mstate.sha3_map
                        for m in map.keys():
                            if m in index:
                ...
                if index.symbolic == False:
                    if index.value in global_state.mstate.sha3_dist.keys():
                        ...
                        if index.value == global_state.mstate.slot:
                            if global_state.mstate.offset != None:
                                if (index.value,state.offset,state.size) not in global_state.world_state.path.store_var_list:
                                    global_state.world_state.path.store_var_list.append((index.value,state.offset,state.size))
                                    global_state.mstate.offset = None
                                    global_state.mstate.size = None
                            else:
                                if (index.value,0,32) not in global_state.world_state.path.store_var_list:
                                    global_state.world_state.path.store_var_list.append((index.value,0,32))
                        else:
                            if (index.value,0,32) not in global_state.world_state.path.store_var_list:
                                global_state.world_state.path.store_var_list.append((index.value,0,32))
                else:
    ...
    def jumpi_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if negated_cond:
            ...
            if global_state.firstexec:
                new_state.world_state.path.pc_list.append(new_state.mstate.pc)
            if global_state.firstexec == None:
                new_state.world_state.path.append(new_state.mstate.pc)
            ...
            if global_state.firstexec:
                ...
                new_state.mstate.depth += 1
                new_state.mstate.pc += 1
                new_state.world_state.constraints.append(True)
                new_state.world_state.path.unreachable_flag = True
                new_state.world_state.path.pc_list.append(new_state.mstate.pc)
                states.append(new_state)
        ...
        if instr["opcode"] == "JUMPDEST":
            if global_state.firstexec:
                global_state.mstate.jumpi = True
                instructions = global_state.environment.code.instruction_list
                if global_state.mstate.jumpi_flag and \
                   ((instructions[global_state.mstate.pc-1]['opcode'][:4] == 'PUSH' and \
                   instructions[global_state.mstate.pc-2]['opcode'] == 'ISZERO' and \
                   instructions[global_state.mstate.pc-3]['opcode'] == 'JUMPDEST') or \
                   (instructions[global_state.mstate.pc-1]['opcode'][:4] == 'PUSH' and \
                   instructions[global_state.mstate.pc-2]['opcode'] == 'ISZERO' and \
                   instructions[global_state.mstate.pc-3]['opcode'][:3] == 'DUP' and \
                   instructions[global_state.mstate.pc-4]['opcode'] == 'JUMPDEST')):
                    states.pop()
            if positive_cond:
                new_state = copy(global_state)
                # add JUMPI gas cost
                ...
                if global_state.firstexec:
                    new_state.world_state.path.pc_list.append(index)
                if global_state.firstexec == None:
                    new_state.world_state.path.append(index)
                states.append(new_state)
            else:
                log.debug("Pruned unreachable states.")
                if global_state.firstexec:
            ...
            if global_state.firstexec:
                if global_state.mstate.jumpi_flag and \
                   (instructions[global_state.mstate.pc-1]['opcode'][:4] == 'PUSH' and \
                   instructions[global_state.mstate.pc-2]['opcode'] == 'ISZERO' and \
                   instructions[global_state.mstate.pc-3]['opcode'] == 'ISZERO' and \
                   instructions[global_state.mstate.pc-4]['opcode'] == 'JUMPDEST'):
                    states.pop()
                if global_state.mstate.pc == 7:
                    states.pop()
            if global_state.firstexec == None:
                if global_state.mstate.pc == 7:
                    states.pop()
